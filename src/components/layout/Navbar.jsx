import React from 'react';
import Link from 'next/link';

const Navbar = ({ onGuideClick }) => {
  // Remove tutorial record clearing function
  // Add feedback functionality
  const handleFeedback = () => {
    window.open('https://github.com/TheGreatCorrine/json-schema-normalization-demo/issues/new?title=Feedback%20on%20JSON%20Schema%20Normalizer&labels=feedback', '_blank');
  };

  return (
    <header className="fixed top-0 left-0 right-0 bg-white border-b shadow-md z-50">
      <div className="container mx-auto px-6">
        <div className="grid grid-cols-3 items-center h-20">
          <div className="flex items-center">
            <div className="flex items-center navbar-logo">
              <img 
                src={process.env.NODE_ENV === 'production' 
                  ? '/json-schema-normalization-demo/logo.png'  // Production path
                  : '/logo.png'                                 // Development path
                } 
                alt="JSON Schema" 
                style={{ height: '40px', width: 'auto' }} 
              />
            </div>
          </div>
          
          <div className="flex justify-center navbar-title">
            <div className="text-2xl font-bold text-gray-900">
              JSON Schema <span className="text-[#0c4da2]">Normalization</span>
            </div>
          </div>
          
          <div className="flex items-center justify-end space-x-4">
            <button 
              onClick={onGuideClick}
              className="text-gray-700 hover:text-gray-900 font-medium guide-button"
            >
              Guide
            </button>
            
            <button 
              onClick={handleFeedback}
              className="text-gray-700 hover:text-gray-900 font-medium feedback-button"
              title="Provide feedback"
            >
              Feedback
            </button>
            
            <button className="p-2 rounded-full hover:bg-gray-100 search-button">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
              </svg>
            </button>
            
            <a 
              href="https://github.com/TheGreatCorrine/json-schema-normalization-demo" 
              className="p-2 text-gray-700 hover:text-[#0c4da2] transition-colors github-link"
              target="_blank" 
              rel="noopener noreferrer"
              aria-label="GitHub repository"
            >
              <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path fillRule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" clipRule="evenodd" />
              </svg>
            </a>
          </div>
        </div>
      </div>
    </header>
  );
};

export default Navbar; 